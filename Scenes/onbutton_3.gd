...
signal four_to_two_on
signal four_to_two_off
signal four_to_three_on
signal four_to_three_off
...
func _on_on_area_shape_body_entered(body: Node3D) -> void:
	if (onCount == 0):
		...
		emit_signal("four_to_two_on")
		emit_signal("four_to_three_on")
	_on_touch()
...
func _on_off_area_shape_body_entered(body: Node3D) -> void:
	if (offCount == 0):
		...
		emit_signal("four_to_two_off")
		emit_signal("four_to_three_off")
	_off_touch()
